...
class UserTest(unittest.TestCase):
    ...
    def test_login_user(self):
        ...
        # check the session is set - have to change the context to homepage first
        with self.app as c:
            rv = c.get('/')
    ...
    def test_edit_profile(self):
        ...
        # check new email is in change confirguration
        ...
        # try to edit with same email
        user = self.user_dict()
        user['email'] = "test@example.com"
        rv = self.app.post('/edit', data=user)
        assert "Email already exists" in str(rv.data)
        
        # try to edit with same username
        user = self.user_dict()
        user['username'] = "TestUsername"
        rv = self.app.post('/edit', data=user)
        assert "Username already exists" in str(rv.data)
        
        # try to edit user1 with user2
        # user1 = self.user_dict()
        # user2 = self.user_dict()
        # # login user2
        # rv = self.app.post('/login', data=dict(
        ...
        #     ))
        # # check that user2 does not have edit button on user1 profile
        # rv = self.app.get('/' + user1['username'])
        # assert "Edit Profile" in str(rv.data)
    ...
    def test_forgot_password(self):
        ...
        # try wrong user name
        rv = self.app.post('/password_reset/nonexistent' + password_reset_code)
        assert rv.status_code == 404
        
        # try wrong code
        rv = self.app.post('/password_reset/' + self.user_dict().get('username') + '/bad-code')
        assert rv.status_code == 404
        ...
        # check the session is set after logging in 
        with self.app as c:
            rv = c.get('/')
            assert session.get('username') == self.user_dict()['username']
